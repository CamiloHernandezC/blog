<div id="aboutUs">
    <h1 class="text-center mt-5">{{ __('general.Who_we_are') }} </h1>

    <div class="d-block d-lg-flex">
        <div style="min-height: 100vh" class="mt-5 w-100 w-lg-50">

            <div style="image-rendering: pixelated; background: url('../img/meli.jpg') no-repeat center center; background-size: 200%; background-position-x: -80px; background-position-y: -38px " class="floating-card floating-img d-block circle profile-image ml-auto mr-auto position-relative" alt="profile image"></div>
            <div class="floating-card floating-img-body w-75 pb-4 px-4 mx-auto mb-4 br-20">
                <h1 class="font-jsff m-auto" style="width: fit-content">Meli</h1>
                <div class="mt-3 text-justify">
                    <p>Soy contadora de profesión pero viajera por pasión, realmente  es una de las cosas que más disfruto, soy la guía turística oficial del equipo, disfruto de la planeación, ejecución y documentación de los viajes así que espero poder compartirles muchas experiencias y como lo decíamos aciertos y desaciertos que hemos tenido en nuestros viajes a través de los años.</p>
                    <p>Soy una apasionada por la lectura especialmente de crecimiento personal y alimentación y en los últimos meses por situaciones personales que he atravesado me he apasionado aún más y he encontrado diferentes textos que me han ayudado afrontar estas situaciones y espero poder compartirlo con ustedes.</p>
                    <p>Y para no hacerles más largo el cuento ya que nos iremos conociendo en el camino otros temas que disfruto son los deportes, la moda y el maquillaje los cuales también podrán encontrar en este espacio</p>
                </div>
            </div>
        </div>
        <div style="min-height: 100vh" class="mt-5 w-100 w-lg-50">

            <div style="image-rendering: pixelated; background: url('../img/cami.jpg') no-repeat center center; background-size: cover;" class="floating-card floating-img d-block circle profile-image ml-auto mr-auto position-relative" alt="profile image"></div>
            <div class="floating-card floating-img-body w-75 pb-4 px-4 mx-auto mb-4 br-20">
                <h1 class="font-jsff m-auto" style="width: fit-content">Cami</h1>
                <div class="mt-3 text-justify">
                    <p>Yo soy un apasionado por el emprendimiento, por eso junto a meli creamos esta plataforma donde compartiremos nuestros gustos con ustedes.</p>
                    <p>Me verán hablar no solo de emprendimiento sino de finanzas, compartiendoles cada truco que use, tecnología -en especial del desarrollo de software que es en lo que trabajo profesionalmente y de Inteligencia Artifical que es un tema que me interesa demasido-</p>
                    <p>Incluso podames disctuir sobre series, películas, libros y hasta hablar de deportes, aun que más que discutir los marcadores o hacer apuestas deportivas me gusta practicar diferentes actividades y como soy una persona muy curiosa me encanta también aprender y <strong>disfrutar de cada instante.</strong></p>
                </div>
            </div>
        </div>
    </div>
</div>
